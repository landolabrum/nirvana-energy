...
import styles from './CanopyNav.scss';
...
import { defaultOverlayFor, OVERLAY_TYPES, OverlayType } from '../../CanopyOverlays/canopyOverlayTypes';
...
const TZ_OPTS = [
  'UTC', 'America/Denver', 'America/Los_Angeles', 'America/Chicago',
...
// alert(event.ends_at)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updated =
        (Array.isArray((res as any)?.data) ? (res as any).data[0] : (res as any)?.data) ?? { ...event, ...payload };
      router.reload();
      setNotification({
        active: true, dismissable: true, persistence: 3500,
        list: [{ label: 'Event updated', message: `“${updated.name}” • ${updated.timezone ?? '—'} • live: ${updated.is_live ? 'Yes' : 'No'}` }],
      });
      onDone(updated);
    } catch (e: any) {
  ...
  return (
    <div style={{ minWidth: 360 }}>
      ...
      <UiForm fields={fields} onChange={handleChange} onSubmit={handleSubmit} submitText="Save changes" loading={busy} />
    </div>
  );
};
...
const CanopyNavContent: React.FC<Props> = ({ current, events, setCurrent, onDelete, deletingId }) => {
  const router = useRouter();
          ...
          statements: [
            {
              ...
              onClick: () => {
                ...
                try { window.sessionStorage.setItem(ASK_KEY, '1'); } catch { }
                closeModal();
              },
            },
            {
  ...
  const overlaysLsKey = useMemo(
    () => (eventId ? `${LS_OVERLAY_PREFIX}${eventId}` : undefined),
    [eventId]
  );

  const { overlays: storeOverlays, setOverlays } = useOverlayStore(eventId, current?.name);
  const { localItem: overlaysLocal, setLocalItem: setLsOverlays } = useLocalStorage(overlaysLsKey);

  // Prefer store; fallback to LS shape
  const overlaysArr: Overlay[] = useMemo(() => {
    if (Array.isArray(storeOverlays)) return storeOverlays as any[];
    if (Array.isArray(overlaysLocal)) return overlaysLocal as any[];
    if (overlaysLocal && Array.isArray((overlaysLocal as any).value)) return (overlaysLocal as any).value;
    return [];
  }, [storeOverlays, overlaysLocal]);

  // Hydrate store from LS whenever event or LS payload changes
  useEffect(() => {
    if (!eventId) return;
    const lsList = Array.isArray(overlaysLocal)
      ? overlaysLocal
      : (overlaysLocal && Array.isArray((overlaysLocal as any).value)) ? (overlaysLocal as any).value : [];
    const same = JSON.stringify(storeOverlays ?? []) === JSON.stringify(lsList ?? []);
    if (!same) setOverlays(() => lsList);
  }, [eventId, overlaysLocal, setOverlays]); // storeOverlays intentionally omitted
  ...
  const toggleOverlay = useCallback((type: OverlayType) => {
    if (!eventId || !overlaysLsKey) return;
    const key = String(type).toLowerCase();

    setOverlays(prev => {
      const list = Array.isArray(prev) ? [...prev] : [];

      const idx = list.findIndex(o => String(o?.type).toLowerCase() === key);
      ...
      // Mirror to LS so other tabs/Undo/Push flows see it
      setLsOverlays(overlaysLsKey, next);

      return next;
    });
  }, [eventId, overlaysLsKey, setOverlays, setLsOverlays, current?.name]);
  ...
  const onShareLive = useCallback(() => {
    ...
    const doCopy = async () => {
      try {
        ...
        setNotification({
          active: true, dismissable: true, persistence: 2000,
          ...
          <div style={{ display: 'flex', gap: 'var(--s-9)', marginTop: 'var(--s-9)' }}>
            <a href={link} target="_blank" rel="noreferrer">
              <UiButton variant="success">
                <UiIcon icon="fa-up-right-from-square" />&nbsp;Open in new tab
              </UiButton>
            </a>
            <UiButton variant="flat" onClick={() => closeModal()}>
              <UiIcon icon="fa-xmark" />&nbsp;Close
            </UiButton>
          </div>
  ...
  }, [current?.id, openModal, closeModal, setNotification, merchant?.mid]);
  ...
  const onSelect = useCallback(
    (opt: { value?: string } | string) => {
      ...
      setNotification({
        active: true, persistence: 2000, dismissable: true,
  ...
  /* -------- render -------- */
  const EVENT_TABLE_KEYS = [
    'starts_at', 'ends_at', 'timezone', 'is_live'] as const;
  ...
  return (
    <>
      ...
      <div className="admin-live-stream-nav">
        <fieldset className="admin-live-stream-nav__section">
          <legend>events</legend>
          <div className="admin-live-stream-nav__actions">
            <div className="admin-live-stream-nav__actions--list">
              {/* Share */}
              <UiIcon
                icon="fa-arrow-up-from-bracket"
                onClick={current ? onShareLive : undefined}
              />
                          ...
                          <EditEventForm
                            ...
                            onDone={(updated) => {
                              if (updated) {
                                ...
                                setCurrent(merged);
                                router.push({ pathname: router.pathname, query: { ...router.query, event: merged.id } });
                              }
                              closeModal();
                            }}
                          />
...
<UiIcon
  ...
  onClick={
    current
      ? () => {
          const label = current?.name ?? 'this event';
          openModal({
            confirm: {
              ...
              statements: [
                {
                  ...
                  variant: 'danger',
                  onClick: async () => {
                    // Call onDelete to trigger deletion and close modal
              ...
              body: 'This action cannot be undone. All data associated with this event may be removed.',
            },
          });
        }
      : undefined
...
/>
        ...
        {/* Overlays (buttons reflect store+LS truth immediately) */}
        <fieldset className="admin-live-stream-nav__section">
          <legend>overlays</legend>
          <div className="admin-live-stream-nav__overlay-list">
            {OVERLAY_TYPES.map(({ type, label, icon }: any) => {
              const enabled = isTypeEnabled(type);
              return (
                <UiButton
                  key={type}
                  variant={enabled ? 'success' : 'flat'}
                  onClick={() => current && toggleOverlay(type)}
                >
                  <UiIcon icon={icon} />&nbsp;{label}{enabled ? ' ✓' : ''}
                </UiButton>
              );
            })}
          </div>
          {!current && <div className="admin-live-stream-nav__hint">select an event to enable overlays</div>}
        </fieldset>
      </div>
    </>
  );
      ...
      <>
        <div className="s-w-9">
          <UiCollapse label="settings">
            <CanopyNavContent {...props} />
          </UiCollapse>
        </div>
      </>
    );
  return <CanopyNavContent {...props} />;
};
