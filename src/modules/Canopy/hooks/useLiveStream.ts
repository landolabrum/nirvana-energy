...
import { CanonOverlay, normalizeOverlayArray, jsonStable } from "../views/CanopyOverlays/canopyOverlayTypes";
...
export function useLiveStream() {
  ...
  const deleteEvent = useCallback(async (id: string): Promise<boolean> => {
    try {
      await db.deleteData({ tableName: TABLE_EVENT, where: { exact: { id } } });
      await db.deleteData({ tableName: TABLE_OVERLAY, where: { exact: { event_id: id } } });
      await db.deleteData({ tableName: TABLE_ROSTER,  where: { exact: { event_id: id } } });
      return true;
    } catch (error) {
