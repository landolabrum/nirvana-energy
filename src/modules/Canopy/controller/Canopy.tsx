...
import AdminLiveStreamView from '../views/CanopyView/controller/CanopyView';
import useLocalStorage from '@webstack/hooks/storage/useLocalStorage';
import { useRouter } from 'next/router';
import CanopyNav from '../views/CanopyNav/controller/CanopyNav';
import { useLiveStream } from '../hooks/useLiveStream';
import type { EventRow } from '../hooks/useLiveStream';
// import { useCinemaLayout } from '../functions/cinamaLayout';
...
const AdminLiveStream: React.FC = () => {
  // useCinemaLayout('linear-gradient(to left, var(--gray-80),var(--gray-90))');

  const router = useRouter();
  const { openModal, closeModal } = useModal();
  const [, setNotification] = useNotification();
  const { events, loadEvents, deleteEvent: deleteEventDb, getOverlaysById } = useLiveStream();

  const vid = useMemo(() => {
    if (!router?.isReady) return undefined;
    const v = router.query?.vid as string | undefined;
  ...
  const setUrlEvent = useCallback((id: string | null) => {
    if (!router?.isReady) return;
  ...
  const setLsCurrent = useCallback((id: string | null) => {
    try {
  ...
  const handleUserSelect = useCallback((evt: EventRow | null) => {
  ...
  const deleteEvent = useCallback(async (id?: string) => {
    ...
    const label = events?.find(e => String(e.id) === String(targetId))?.name ??
      (current?.id === targetId ? current?.name : undefined) ?? 'this event';
  ...
  }, [current?.id, current?.name, events, deleteEventDb, loadEvents, setUrlEvent, setLsCurrent, openModal, closeModal, setNotification]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = getOverlayStream(String(current.id), overlayUrl, () => {
      ...
      inFlight = true;
      Promise.resolve(
        getOverlaysById(String(current.id), { force: true })
      ).finally(() => {
        inFlight = false;
        lastRun = Date.now();
      });
...
platform='instagram'
 user={user}
/> */}
        <span className='error notice '>Notice from the developer: if it glitches, refresh the page. or
          <UiButton onClick={callLando} variant="link">more help</UiButton>
        </span>

        <div className="canopy__header">
          <div className="canopy__header--title">
            <div>Canopy</div>
            <p>{current?.name || "LiveStream Visual Controller"}</p>
          </div>
        </div>
        ...
        <div className="canopy__content">
          <div className="canopy--nav">
            <CanopyNav
              ...
              onDelete={(e?: string) => { void deleteEvent(e); }}
              deletingId={deletingId ?? undefined}
            />
          </div>
          <div className="canopy--view" aria-busy={!!deletingId}>
            <AdminLiveStreamView current={current} vid={vid} />
          </div>
        </div>
      </div>
    </>
