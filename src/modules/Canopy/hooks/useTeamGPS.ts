...
export function useTeamGpsByVehicleNumber(
  ...
  const fetchOnce = useCallback(async () => {
    ...
    try {
      ...
      const rows: GpsRow[] = Array.isArray(res?.data) ? res.data : [];
      const next = new Map<string, string>();

      // limit optional set to just the boat numbers we’re about to display
      const want = new Set((vehicleNumbers ?? []).map((v) => String(v)));
      ...
      for (const r of rows) {
        ...
        if (!vehNum) continue;
        if (want.size && !want.has(vehNum)) continue;
  ...
  }, [db, eventId, vehicleNumbers, roster]);
