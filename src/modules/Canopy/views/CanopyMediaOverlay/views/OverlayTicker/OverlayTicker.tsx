...
const OverlayTicker: React.FC<OverlayTickerProps> = ({
 ...
 useLayoutEffect(() => {
  ...
  const measure = () => {
    // Measure EXACT content width of the measured row
  ...
  const renderItem = (it: TickerItem, key: React.Key) => {
    ...
    // Plain string/number -> UiMarkdown
    ...
    // UiMarkdownProps with per-item styling
  ...
  <div className="overlay-ticker__track" aria-hidden={rowWidth <= 0}>
    ...
    <span key="row-clone-wrap">
      <div className="overlay-ticker__row overlay-ticker__row--clone" aria-hidden="true">
