...
//   const [currentLight, setCurrentLight] = useState<ILightDisplay | undefined>();
//   const [hueData, setHueData] = useState<ILightDisplay[]>([]);
//   const homeService = getService<IHomeService>('IHomeService');
//   const group = useRef<string[]>([]);

//   const updateLight = (changedLight: ILightDisplay, isColor?: boolean) => {
//     setHueData((prevData) =>
//       prevData.map((light) =>
//         light.id_ === changedLight.id_
//           ? { ...changedLight, view: isColor ? 'color' : light.view }
//           : light
//       )
//     );
//   };

//   const multiHomeService = useCallback(async (action: string, data: any) => {
//     setLoader(true);
...
//       switch (action) {
//         case 'toggle':
//           response = await homeService.hue_toggle(numericId);
//           break;
//         case 'brightness':
//           response = await homeService.hue_brightness(numericId, data.bri);
//           break;
//         case 'color':
//           response = await homeService.lightColor(numericId, data.hex);
//           const calculatedHex = calculateHexFromHueSatBri(
//             response.hue,
//             response.sat,
//             response.bri
//           );
...
//           return;
//       }
//       if (response) updateLight(response, action === 'color');
//     };

//     for (const id_ of group.current) {
//       await applyAction(id_);
//     }
//     setLoader(false);
//   }, [homeService]);

//   const hueList = useCallback(async () => {
//     setLoader(true);
//     try {
//       const response = await homeService.hue_list();
//       setHueData(response.map((light) => ({
//         ...light,
//         hex: calculateHexFromHueSatBri(light.hue, light.sat, light.bri),
//       })));
//     } catch (error) {
//       console.error('[ FETCH LIGHTS ERROR ]', error);
//     } finally {
//       setLoader(false);
//     }
//   }, [homeService]);

//   const handleLightAnimation = useCallback((light: ILightDisplay, color: string) => {
//     setCurrentLight(light);
//     multiHomeService('color', { id: light.id_, hex: color });
//   }, [multiHomeService]);
...
//                 Math.random() * 10 - 5,
//               ]}
//               onClick={() => handleLightAnimation(light, '#ff0000')}
//             >
//               <sphereGeometry args={[0.5, 32, 32]} >
//               </sphereGeometry>
//               <meshStandardMaterial color={light.hex || '#ffffff'} />
//               <Html>
